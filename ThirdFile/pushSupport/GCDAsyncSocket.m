	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		if (enableIPv4)
		{
			...
			dispatch_source_set_event_handler(accept4Source, ^{ @autoreleasepool {
				...
				NSLog(@"numPendingConnections: %lu", numPendingConnections);
			...
			dispatch_source_set_cancel_handler(accept4Source, ^{
				...
				#if !OS_OBJECT_USE_OBJC
				NSLog(@"dispatch_release(accept4Source)");
			...
			NSLog(@"dispatch_resume(accept4Source)");
		...
		if (enableIPv6)
		{
			...
			dispatch_source_set_cancel_handler(accept6Source, ^{
				...
				#if !OS_OBJECT_USE_OBJC
				NSLog(@"dispatch_release(accept6Source)");
				dispatch_release(acceptSource);
				#endif
				
				NSLog(@"close(socket6FD)");
				close(socketFD);
			});
			
			NSLog(@"dispatch_resume(accept6Source)");
...
- (BOOL)doAccept:(int)parentSocketFD
{
	...
	if (parentSocketFD == socket4FD)
	{
		...
		if (childSocketFD == -1)
		{
			NSLog(@"Accept failed with error: %@", [self errnoError]);
			return NO;
		}
	...
	if (result == -1)
	{
		NSLog(@"Error enabling non-blocking IO on accepted socket (fcntl)");
		return NO;
	}
...
- (void)lookup:(int)aConnectIndex didSucceedWithAddress4:(NSData *)address4 address6:(NSData *)address6
{
	...
	{
		NSLog(@"Ignoring lookupDidSucceed, already disconnected");
...
- (void)lookup:(int)aConnectIndex didFail:(NSError *)error
{
	...
	if (aConnectIndex != connectIndex)
	{
		NSLog(@"Ignoring lookup:didFail: - already disconnected");
...
- (BOOL)connectWithAddress4:(NSData *)address4 address6:(NSData *)address6 error:(NSError **)errPtr
{
	...
	NSLog(@"IPv4: %@:%hu", [[self class] hostFromAddress:address4], [[self class] portFromAddress:address4]);
	NSLog(@"IPv6: %@:%hu", [[self class] hostFromAddress:address6], [[self class] portFromAddress:address6]);
...
- (void)didConnect:(int)aConnectIndex
{
	...
	if (aConnectIndex != connectIndex)
	{
		NSLog(@"Ignoring didConnect, already disconnected");
...
- (void)didNotConnect:(int)aConnectIndex error:(NSError *)error
{
	...
	if (aConnectIndex != connectIndex)
	{
		NSLog(@"Ignoring didNotConnect, already disconnected");
...
- (void)startConnectTimeout:(NSTimeInterval)timeout
{
	if (timeout >= 0.0)
	{
		...
		dispatch_source_set_cancel_handler(connectTimer, ^{
			NSLog(@"dispatch_release(connectTimer)");
...
- (void)closeWithError:(NSError *)error
{
	...
	else
	{
		if (accept4Source)
		{
			NSLog(@"dispatch_source_cancel(accept4Source)");
		...
		if (accept6Source)
		{
			NSLog(@"dispatch_source_cancel(accept6Source)");
		...
		if (readSource)
		{
			NSLog(@"dispatch_source_cancel(readSource)");
		...
		if (writeSource)
		{
			NSLog(@"dispatch_source_cancel(writeSource)");
...
- (void)setupReadAndWriteSourcesForNewlyConnectedSocket:(int)socketFD
{
	...
	dispatch_source_set_event_handler(readSource, ^{ @autoreleasepool {
		...
		NSLog(@"readEventBlock");
		
		socketFDBytesAvailable = dispatch_source_get_data(readSource);
		NSLog(@"socketFDBytesAvailable: %lu", socketFDBytesAvailable);
	...
	dispatch_source_set_cancel_handler(readSource, ^{
		...
		NSLog(@"readCancelBlock");
		
		#if !OS_OBJECT_USE_OBJC
		NSLog(@"dispatch_release(readSource)");
	...
	dispatch_source_set_cancel_handler(writeSource, ^{
		...
		NSLog(@"writeCancelBlock");
		
		#if !OS_OBJECT_USE_OBJC
		NSLog(@"dispatch_release(writeSource)");
	...
	NSLog(@"dispatch_resume(readSource)");
...
- (void)suspendReadSource
{
	...
	{
		NSLog(@"dispatch_suspend(readSource)");
...
- (void)suspendWriteSource
{
	...
	{
		NSLog(@"dispatch_suspend(writeSource)");
...
- (void)resumeWriteSource
{
	if (flags & kWriteSourceSuspended)
	{
		NSLog(@"dispatch_resume(writeSource)");
                        ...
                        tag:(long)tag
{
	if (offset > [buffer length]) {
		NSLog(@"Cannot read: offset > [buffer length]");
		return;
	}
...
{
	if ([data length] == 0) {
		NSLog(@"Cannot read: [data length] == 0");
		return;
	}
	if (offset > [buffer length]) {
		NSLog(@"Cannot read: offset > [buffer length]");
	...
	if (maxLength > 0 && maxLength < [data length]) {
		NSLog(@"Cannot read: maxLength > 0 && maxLength < [data length]");
		return;
	}
...
- (void)maybeDequeueRead
{
	NSLog(@"%s\ncurrentRead = %@\tflags has kConnected : %d", __func__, currentRead, flags & kConnected);
	NSAssert(dispatch_get_specific(IsOnSocketQueueOrTargetQueueKey), @"Must be dispatched on socketQueue");
	...
	if ((currentRead == nil) && (flags & kConnected))
	{
		if ([readQueue count] > 0)
		{
			...
			{
				NSLog(@"Dequeued GCDAsyncReadPacket");
...
- (void)flushSSLBuffers
{
	...
	if ([self usingCFStreamForTLS])
	{
		...
		{
			NSLog(@"%s - Flushing ssl buffers into prebuffer...", __func__);
			...
			NSLog(@"%s - CFReadStreamRead(): result = %i", __func__, (int)result);
	...
	if (estimatedBytesAvailable > 0)
	{
		...
		do
		{
			NSLog(@"%s - estimatedBytesAvailable = %lu", __func__, (unsigned long)estimatedBytesAvailable);
			...
			NSLog(@"%s - read from secure socket = %u", __func__, (unsigned)bytesRead);
			...
			NSLog(@"%s - prebuffer.length = %zu", __func__, [preBuffer availableBytes]);
...
- (void)doReadData
{
	...
	{
		NSLog(@"No currentRead or kReadsPaused");
	...
	{
		NSLog(@"No data available to read...");
	...
	{
		NSLog(@"Waiting for SSL/TLS handshake to complete");
	...
	if ([preBuffer availableBytes] > 0)
	{
		...
		NSLog(@"copied(%lu) preBufferLength(%zu)", (unsigned long)bytesToCopy, [preBuffer availableBytes]);
	...
	if (!done && !error && !socketEOF && !waiting && hasBytesAvailable)
	{
        ...
        NSLog(@"%s\nreadIntoPreBuffer = %d", __func__, readIntoPreBuffer);
		...
		if (flags & kSocketSecure)
		{
			if ([self usingCFStreamForTLS])
			{
				...
				NSLog(@"CFReadStreamRead(): result = %i", (int)result);
			...
			else
			{
				...
				do
				{
					...
					NSLog(@"read from secure socket = %u", (unsigned)loop_bytesRead);
		...
		else
		{
			...
			NSLog(@"%s\nread from socket = %i", __func__, (int)result);
		...
		if (bytesRead > 0)
		{
			...
			else if (currentRead->term != nil)
			{
				...
				if (readIntoPreBuffer)
				{
					...
					[preBuffer didWrite:bytesRead];
					NSLog(@"read data into preBuffer - preBuffer.length = %zu", [preBuffer availableBytes]);
					
					// Search for the terminating sequence
					
					bytesToRead = [currentRead readLengthForTermWithPreBuffer:preBuffer found:&done];
					NSLog(@"copying %lu bytes from preBuffer", (unsigned long)bytesToRead);
					...
					NSLog(@"preBuffer.length = %zu", [preBuffer availableBytes]);
				...
				else
				{
					...
					else if (overflow > 0)
					{
						...
						NSLog(@"copying %ld overflow bytes into preBuffer", (long)overflow);
...
- (void)doReadEOF
{
	...
	{
		NSLog(@"Socket reached EOF, but there is still data available in prebuffer");
...
- (void)setupReadTimerWithTimeout:(NSTimeInterval)timeout
{
	if (timeout >= 0.0)
	{
		...
		dispatch_source_set_cancel_handler(readTimer, ^{
			NSLog(@"dispatch_release(readTimer)");
...
- (void)maybeDequeueWrite
{
	NSLog(@"%s\ncurrentWrite = %@\tflags has kConnected : %d", __func__, currentWrite, flags & kConnected);
	NSAssert(dispatch_get_specific(IsOnSocketQueueOrTargetQueueKey), @"Must be dispatched on socketQueue");
...
- (void)doWriteData
{
	...
	{
		NSLog(@"No currentWrite or kWritesPaused");
	...
	if (!(flags & kSocketCanAcceptBytes))
	{
		NSLog(@"No space available to write...");
	...
	if (flags & kSocketSecure)
	{
		if ([self usingCFStreamForTLS])
		{
			...
			NSLog(@"CFWriteStreamWrite(%lu) = %li", (unsigned long)bytesToWrite, result);
	...
	else
	{
		...
		NSLog(@"wrote to socket = %zd", result);
	...
	if (bytesWritten > 0)
	{
		...
		NSLog(@"currentWrite->bytesDone = %lu", (unsigned long)currentWrite->bytesDone);
...
- (void)setupWriteTimerWithTimeout:(NSTimeInterval)timeout
{
	if (timeout >= 0.0)
	{
		...
		dispatch_source_set_cancel_handler(writeTimer, ^{
			NSLog(@"dispatch_release(writeTimer)");
...
{
	NSLog(@"sslReadWithBuffer:%p length:%lu", buffer, (unsigned long)*bufferLength);
	
	if ((socketFDBytesAvailable == 0) && ([sslPreBuffer availableBytes] == 0))
	{
		NSLog(@"%s - No data available to read...", __func__);
	...
	{
		NSLog(@"%s: Reading from SSL pre buffer...", __func__);
		...
		NSLog(@"%s: Copying %zu bytes from sslPreBuffer", __func__, bytesToCopy);
		
		memcpy(buffer, [sslPreBuffer readBuffer], bytesToCopy);
		[sslPreBuffer didRead:bytesToCopy];
		
		NSLog(@"%s: sslPreBuffer.length = %zu", __func__, [sslPreBuffer availableBytes]);
		...
		if (done) NSLog(@"%s: Complete", __func__);
	}
	...
	{
		NSLog(@"%s: Reading from socket...", __func__);
		...
		if (socketFDBytesAvailable > totalBytesLeftToBeRead)
		{
			...
			NSLog(@"%s: Reading into sslPreBuffer...", __func__);
		...
		else
		{
			...
			NSLog(@"%s: Reading directly into dataBuffer...", __func__);
		...
		NSLog(@"%s: read from socket = %zd", __func__, result);
		
		if (result < 0)
		{
			NSLog(@"%s: read errno = %i", __func__, errno);
		...
		else
		{
			...
			if (readIntoPreBuffer)
			{
				...
				NSLog(@"%s: Copying %zu bytes out of sslPreBuffer", __func__, bytesToCopy);
...
- (void)ssl_startTLS
{
	...
	NSLog(@"Starting TLS (via SecureTransport)...");
...
//	}
	...
	#if TARGET_OS_IPHONE
	{
		...
		if (sslLevel)
		{
			...
			{
				NSLog(@"kCFStreamSSLLevel security option ignored. Overriden by "
			...
			else
			{
				...
				{
					NSLog(@"Unable to match kCFStreamSSLLevel security option to valid SSL protocol min/max");
				}
			}
		}
...
- (void)cf_startTLS
{
	...
	NSLog(@"Starting TLS (via CFStream)...");
	...
	NSLog(@"Waiting for SSL Handshake to complete...");
}
	...
	switch(type)
	{
		case kCFStreamEventHasBytesAvailable:
		{
			dispatch_async(asyncSocket->socketQueue, ^{ @autoreleasepool {
				...
				NSLog(@"CFReadStreamCallback - HasBytesAvailable");
		...
		default:
		{
			...
			dispatch_async(asyncSocket->socketQueue, ^{ @autoreleasepool {
				...
				NSLog(@"CFReadStreamCallback - Other");
	...
	switch(type)
	{
		case kCFStreamEventCanAcceptBytes:
		{
			dispatch_async(asyncSocket->socketQueue, ^{ @autoreleasepool {
				...
				NSLog(@"CFWriteStreamCallback - CanAcceptBytes");
		...
		default:
		{
			...
			dispatch_async(asyncSocket->socketQueue, ^{ @autoreleasepool {
				...
				NSLog(@"CFWriteStreamCallback - Other");
...
- (BOOL)registerForStreamCallbacksIncludingReadWrite:(BOOL)includeReadWrite
{
	NSLog(@"%s %@", __func__, (includeReadWrite ? @"YES" : @"NO"));
...
- (BOOL)addStreamsToRunLoop
{
	...
	{
		NSLog(@"Adding streams to runloop...");
...
- (void)removeStreamsFromRunLoop
{
	...
	if (flags & kAddedStreamsToRunLoop)
	{
		NSLog(@"Removing streams from runloop...");
...
- (BOOL)openStreams
{
	...
	{
		NSLog(@"Opening read and write stream...");
...
- (int)socketFD
{
	if (!dispatch_get_specific(IsOnSocketQueueOrTargetQueueKey))
	{
		NSLog(@"%s - Method only available from within the context of a performBlock: invocation", __func__);
		return SOCKET_NULL;
	}
