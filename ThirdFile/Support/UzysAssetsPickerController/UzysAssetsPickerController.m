...
- (void)setupOneMediaTypeSelection
{
    ...
    else
    {
        ...
        else if(_maximumNumberOfSelectionVideo == 0)
        {
            ...
            self.labelSelectedMedia.hidden = NO;
            if(_maximumNumberOfSelection >1)
                self.labelSelectedMedia.text = NSLocalizedStringFromTable(@"请选择图片", @"UzysAssetsPickerController", nil);
            else
                self.labelSelectedMedia.text = NSLocalizedStringFromTable(@"请选择一张图片", @"UzysAssetsPickerController", nil);
        }
        else
        {
