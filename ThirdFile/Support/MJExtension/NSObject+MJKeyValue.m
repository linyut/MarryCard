...
+ (instancetype)objectWithKeyValues:(NSDictionary *)keyValues
{
    if (![keyValues isKindOfClass:[NSDictionary class]]) {
        [NSException raise:@"keyValues is not a NSDictionay Class" format:@"这不是一个字典类型"];
    }
...
+ (NSArray *)keyValuesArrayWithObjectArray:(NSArray *)objectArray
{
    ...
    if (![objectArray isKindOfClass:[NSArray class]]) {
        [NSException raise:@"objectArray is not a NSArray" format:@"不是一个数组类型"];
    }
...
{
    if([keyValuesArray isKindOfClass:[NSNull class]]){
